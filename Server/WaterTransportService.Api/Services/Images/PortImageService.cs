...
public class PortImageService(IEntityRepository<PortImage, Guid> repo) : IImageService<PortImageDto, CreatePortImageDto, UpdatePortImageDto>
{
    ...
    public async Task<(IReadOnlyList<PortImageDto> Items, int Total)> GetAllAsync(int page, int pageSize, CancellationToken ct)
    {
        page = page <= 0 ? 1 : page;
    ...
    public async Task<PortImageDto?> GetByIdAsync(Guid id, CancellationToken ct)
    {
        var e = await _repo.GetByIdAsync(id);
        return e is null ? null : MapToDto(e);
    }

    public async Task<PortImageDto?> CreateAsync(CreatePortImageDto dto, CancellationToken ct)
    {
    ...
    public async Task<PortImageDto?> UpdateAsync(Guid id, UpdatePortImageDto dto, CancellationToken ct)
    {
